...
import json
from .models import UrlSystemapi
import numpy as np
...
# def allSystem(request):
#   mydata = UrlSystemapi.objects.values_list('Url')
#   mydata_count = UrlSystemapi.objects.filter(NameSystemUrl='EXP0', status=0).count()
  
#   print (mydata_count)
#   return allSystem,mydata,mydata_count
...
def url_pic(request,status):
        ...
        mydata = UrlSystemapi.objects.values_list('Url')
        # mydata_count = UrlSystemapi.objects.filter(NameSystemUrl='EXP').count()
        mydata_count = UrlSystemapi.objects.count()
